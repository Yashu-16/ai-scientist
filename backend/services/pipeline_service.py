...
# V3 — ClinicalTrials.gov integration added
# Real clinical trial data enriched into every drug
...
from backend.services.clinical_trials_service  import enrich_drugs_with_trials
...
class PipelineCache:
    ...
    def set(self, disease_name: str, max_targets: int, max_papers: int,
        ...
        key = self._make_key(disease_name, max_targets, max_papers, max_drugs)
        self._cache[key] = {"timestamp": datetime.now(), "data": data}
...
# ── Stage wrappers for parallel execution ────────────────────
...
def compute_decision_summary(result: DiseaseAnalysisResult) -> "DecisionSummary":
    ...
    if drug_phase == 4:
        if risk_level == "High":
            ...
            action = (
                ...
                f"risk. Recommend literature review of combination therapy approaches."
            )
        else:
            action = (
        ...
        action = (
            ...
            f"Recommend reviewing trial results and designing complementary assays."
        )
    elif drug_phase == 2:
        action = (
        ...
        action = (
            ...
            f"targeting {target_protein} and patent landscape analysis."
        )
    ...
    evidence_basis = (
        f"{paper_count} research papers | "
        f"Evidence: {ev_label} | "
        f"Protein score: {best.protein_score:.2f} | "
        f"Drug phase score: {best.drug_score:.2f}"
    )
...
def compute_uncertainty(
    ...
    paper_count = len(result.papers)
    if paper_count < 3:
        paper_impact  = "High";   paper_penalty = 0.35; flags["low_paper_count"] = True
    elif paper_count < 6:
        paper_impact  = "Medium"; paper_penalty = 0.15; flags["low_paper_count"] = False
    else:
        paper_impact  = "Low";    paper_penalty = 0.05; flags["low_paper_count"] = False
    ...
    factors.append(UncertaintyFactor(
        ...
        description = (
            ...
            f"{'Insufficient.' if paper_count < 3 else 'Adequate.' if paper_count < 6 else 'Strong evidence base.'}"
        )
    ))

    # Factor 2: Protein association
    if result.protein_targets:
        top_score = result.protein_targets[0].association_score
        if top_score < 0.6:
            prot_impact  = "High";   prot_penalty = 0.30; flags["weak_protein_assoc"] = True
        elif top_score < 0.75:
            prot_impact  = "Medium"; prot_penalty = 0.15; flags["weak_protein_assoc"] = False
        else:
            prot_impact  = "Low";    prot_penalty = 0.05; flags["weak_protein_assoc"] = False
        ...
        factors.append(UncertaintyFactor(
            ...
            description = (
                ...
                f"{'Weak.' if top_score < 0.6 else 'Moderate.' if top_score < 0.75 else 'Strong association — low uncertainty.'}"
            )
        ))

    # Factor 3: FDA risk
    high_risk_drugs = [d for d in result.drugs if d.risk_level == "High"]
    if high_risk_drugs:
        risk_impact  = "High"; risk_penalty = 0.20; flags["high_fda_risk"] = True
        risk_desc    = (
            f"{len(high_risk_drugs)} high-risk drug(s): "
            f"{', '.join(d.drug_name for d in high_risk_drugs[:2])}."
        )
    else:
        risk_impact  = "Low";  risk_penalty = 0.0;  flags["high_fda_risk"] = False
        risk_desc    = "No high-risk FDA signals detected."

    uncertainty_raw += risk_penalty
    factors.append(UncertaintyFactor(
        factor="FDA Risk Signals", impact=risk_impact, description=risk_desc
    ))

    # Factor 4: Drug coverage
    drug_count = len(result.drugs)
    if drug_count < 2:
        drug_impact  = "High";   drug_penalty = 0.20; flags["limited_drug_data"] = True
    elif drug_count < 4:
        drug_impact  = "Medium"; drug_penalty = 0.10; flags["limited_drug_data"] = False
    else:
        drug_impact  = "Low";    drug_penalty = 0.0;  flags["limited_drug_data"] = False
    ...
    # Factor 5: Causal evidence (hypothesis-level)
    if hypothesis and hasattr(hypothesis, "causal_analysis") and hypothesis.causal_analysis:
        causal_score = hypothesis.causal_analysis.causal_score
        if causal_score < 0.2:
            causal_impact  = "High";   causal_penalty = 0.20; flags["no_causal_evidence"] = True
        elif causal_score < 0.5:
            causal_impact  = "Medium"; causal_penalty = 0.10; flags["no_causal_evidence"] = False
        else:
            causal_impact  = "Low";    causal_penalty = 0.0;  flags["no_causal_evidence"] = False
        ...
        factors.append(UncertaintyFactor(
            ...
            description = (
                ...
                f"{'Correlational only.' if causal_score < 0.2 else 'Some causal signals.' if causal_score < 0.5 else 'Good causal evidence.'}"
            )
        ))
    else:
    ...
    if uncertainty_score >= 0.7:
        ...
        reason = "Multiple critical data gaps. Substantial validation needed."
        note   = "Increase paper count, find stronger causal evidence."
    elif uncertainty_score >= 0.45:
        label = "High"; color = "#f97316"
        reason = "Several uncertainty factors. Results indicative, not definitive."
        note   = "Retrieve more papers and verify protein associations."
    elif uncertainty_score >= 0.25:
        label = "Medium"; color = "#f59e0b"
        reason = "Some uncertainty factors but manageable. Reasonable reliability."
        note   = "Consider expanding literature search."
    else:
        label = "Low"; color = "#22c55e"
        reason = "Strong evidence base. Results reliable for decision-making."
        note   = "Proceed with confidence to experimental validation."
...
def run_data_pipeline(
    ...
    1. Cache check
    2. Proteins (Stage 1)
    3. Drugs + Papers in parallel (Stages 2+3)
    4. Enrich drugs with ClinicalTrials.gov data (Stage 4) ← NEW
    ...
    # ── Stages 2+3: Drugs + Papers in parallel ────────────────
        ...
        for future in as_completed([future_drugs, future_papers]):
            try:
                ...
                if future == future_drugs:
                    ...
                    print(f"  ✅ Drugs complete: {drug_result.get('total_drugs', 0)} found")
                else:
                    paper_result = result_data
                    print(f"  ✅ Papers complete: {len(paper_result.get('papers', []))} found")
            except Exception as e:
    ...
    # ── Stage 4: Enrich drugs with ClinicalTrials.gov ─────────
    if result.drugs:
        print(f"\n🔬 Stage 4/4 — Enriching drugs with ClinicalTrials.gov...")
        try:
            drug_dicts = [d.model_dump() for d in result.drugs]
            enriched   = enrich_drugs_with_trials(
                drugs        = drug_dicts,
                disease_name = disease_name,
                max_per_drug = 3
            )
            # Write trial data back to Drug objects
            for i, drug in enumerate(result.drugs):
                if i < len(enriched):
                    drug.clinical_trials       = enriched[i].get("clinical_trials", [])
                    drug.trial_count           = enriched[i].get("trial_count", 0)
                    drug.active_trial_count    = enriched[i].get("active_trial_count", 0)
                    drug.completed_trial_count = enriched[i].get("completed_trial_count", 0)

            total_trials = sum(d.trial_count for d in result.drugs)
            print(f"  ✅ Clinical trials enriched: {total_trials} total trials found")

        except Exception as e:
            print(f"  ⚠️  Clinical trials enrichment warning: {e}")
    ...
    print(f"   Uncertainty: {ua.uncertainty_label} (score: {ua.uncertainty_score:.3f})")
    for f in ua.factors:
    ...
    total_trials = sum(getattr(d, "trial_count", 0) for d in result.drugs)
...
def build_llm_context(result: DiseaseAnalysisResult) -> str:
    ...
    for drug in result.drugs[:5]:
        ...
        trial_count = getattr(drug, "trial_count", 0)
        if trial_count > 0:
            active    = getattr(drug, "active_trial_count", 0)
            trial_note = f"| Trials: {trial_count} ({active} active)"
        lines.append(
            f"• {drug.drug_name} → {drug.target_gene} | "
            f"Phase {drug.clinical_phase} | "
            f"{drug.mechanism} | Risk: {drug.risk_level} "
            f"{fda_note} {trial_note}"
        )
    lines.append("")
...
if __name__ == "__main__":
    print("🧬 Pipeline V3 — ClinicalTrials.gov Test")
    print("=" * 55)

    t1 = time.time()
    r  = run_data_pipeline("Alkaptonuria", 5, 4, 3)
    print(f"\n⏱️  Time: {round(time.time()-t1, 2)}s")
    print(f"\n📊 Results:")
    print(f"   Proteins : {len(r.protein_targets)}")
    print(f"   Drugs    : {len(r.drugs)}")
    print(f"   Papers   : {len(r.papers)}")
    for drug in r.drugs:
        tc = getattr(drug, "trial_count", 0)
        ac = getattr(drug, "active_trial_count", 0)
        print(f"   💊 {drug.drug_name}: {tc} trials ({ac} active)")
        for trial in getattr(drug, "clinical_trials", [])[:2]:
            print(f"      🔬 {trial['nct_id']} | {trial['status_label']} | {trial['title'][:50]}")